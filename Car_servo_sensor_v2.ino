...
int MotorSpeed =70; //fixed motor speed for now - write this to PWM. will later have this variable rewritten using a controller. If A and B have different speeds, the car will turn.
...
float pos=40; //create a variable that can be adjusted to be the angle of the servo 0-180 only.
...
void forward(){
  if (distance>hit){ //while distance is >15cm, the car will continue moving forward
